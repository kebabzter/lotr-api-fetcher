...
>(
  (
    ...
    const tooltipLabel = React.useMemo(() => {
      ...
      return <p className={cn("font-medium", labelClassName)}>{value}</p>
    }, [
      label,
      labelFormatter,
      payload,
        ...
        <section className="grid gap-1.5">
          {payload.map((item, index) => {
            ...
            return (
              ...
              >
                {!hideIndicator && (
                  <mark className="flex items-center gap-1.5">
                    {indicator === "dot" && (
                      <mark
                    ...
                    {indicator === "dashed" && (
                      <mark
                        className="h-0.5 w-3"
                        style={{
                          backgroundImage: `repeating-linear-gradient(90deg, ${indicatorColor} 0px, ${indicatorColor} 2px, transparent 2px, transparent 4px)`,
                        }}
                      />
                    )}
                  </mark>
...
>(({ payload, verticalAlign, config, className, ...props }, ref) => {
  if (!payload?.length) {
    return null
  }
